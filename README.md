...
<sub>演示页是 2026-08-20 那一版（第七版）的产物。第八版把第④幕换成了一道自己写的例题，
页面上那一幕还是旧版的经典案例。</sub>
...
每一章七幕，顺序固定。**主体落在第③幕与第④幕上**：第③幕把这个对象造出来，
第④幕用一道例题把它讲透。

| 幕 | 它负责什么 | 分量 |
|---|---|---|
| ① 造出缺口 | 先给一个用手上工具够不着的具体情形，让旧办法在眼前断掉 | 中 |
| ② 它从哪里来 | 谁、哪一年、卡在什么问题上。两三句带过 | 短 |
| ③ 造出来 | 每一步说清楚为什么走这一步，**到最后一刻才给它命名** | 重 |
| ④ 一道自己写的例题 | 题干清晰、风格照后面那份作业、跟作业没有关系，讲解照下半那六样走 | **最重** |
| ⑤ 另一种看法 | 同一件事谁给过别的答案，两边差在哪。一两句 | 短 |
| ⑥ 它自己承认的边界 | 什么时候不成立、用错会得到什么；有名有姓的批评一句带过 | 中偏短 |
...
作者 2026-08-22 的原话：**核心在于上半部分的讲解，必须使用例题的形式，
而这个知识点的来源和批评则只需要简单带过就行了。**
所以这一幕从「一个跟本题无关的经典例子」换成一道自己写的例题，
六条要求跟着它走：

| 例题要满足的 | 具体是什么 |
|---|---|
| 题干非常清晰 | 一句情境、给定的量连着单位、取值范围、问什么、分值、小问编号。只看这个框就能动手算 |
| 风格照后面那份作业 | 同一个考试局的措辞、问法、难度与分值写法。TMUA 就是选择题、不按计算器、公式不给 |
| 跟那份作业没有关系 | 换一个情境、一组数据、一个问法。改几个数字就能互相变过去的，重写一道 |
| 只服务刚讲的那个知识点 | 别的东西都要是他手上已经有的。一道题同时用上两个新对象，他分不清哪一步归这一章 |
| 讲解详细程度跟下半对标 | 照下半那六样走，一样不少。下半怎么讲一个小问，这里就怎么讲一个小问 |
| 动笔前去查这一科的真题 | 认出作业是 TMUA 就去查 TMUA 真题。取真题当例题的，年份卷号题号写出来，拿不准写「未能核实」 |

例题占的是上半原来给经典案例、来历与批评的那部分篇幅，
**下半的详细程度不因为它而降低**：把上半整个抹掉，下半仍旧是一题一节、
一个小问一段、六样齐全；下半每个小问的篇幅跟例题在同一个量级上。
知识点本身照旧讲透，第③幕的逐行推导一步不减。
...
## 三种分类指令：bgct / bbct / olct

同一套规格底下有三种产物类型。发细则的时候把指令写在前面，三个里挑一个，
后面还可以再跟一个 `plustts`。**上面那一整套两半的写法记作 `bgct`**，
另外两类拿它当底子改，改的都是上半。
完整规则在 [SKILL.md §1.65](plugins/dialogue-tutor/skills/dialogue-tutor/SKILL.md)。

| 指令 | 寓意 | 上半 · 讲解篇 | 下半 · 做题篇 | 另外要交的 |
|---|---|---|---|---|
| **`bgct`** | background content | 按对象分章，每章七幕齐全，第④幕是一道自己写的例题；一并上传板书的时候，板书当指导材料融进去 | 全额写 | — |
| **`bbct`** | blackboard content | 整个换成对板书内容的讲解；先把板书长图切开、按顺序读、转写，范围守着考纲；例题那一幕照样写；板书盖不住作业的那一份，拿考纲当板书 | 全额写，规格不动 | — |
| **`olct`**（也写作 `onct`） | ONLY content | 不写。只留一个前置块，装这次做题用得上的公式与定义，考纲外的知识点一个也不用 | 全额写 | — |
| **`… plustts`** | 跟在上面三个后面的后缀 | 照它前面那个指令 | 照它前面那个指令 | 一份伴读讲解稿，看着屏幕用 TTS 收听 |

下半那六样在哪一类里都齐全，一题一节，一个小问一段，一个也不漏。

**`plustts` 要的是一份伴读讲解稿。** HTML 照原指令做完之后另交一份纯文本讲稿，
他一边看着屏幕一边用 TTS 收听。三条地板：段落顺序完全跟着 HTML 从上到下的块顺序走，
一块不换位也不合并；耳朵听到的详细程度不低于眼睛看到的，推导表每一行的等号与依据、
表格每一行、定义卡每一行、题干、得分位置，一样都念到；一块也别漏。
再多一件这一版新长出来的工作，引导视觉：每一块开口第一句先用颜色、形状、位置、
标题原话把他的眼睛放到位。理由写在规格里，他有干眼症，眼睛做不到长时间盯着看，
所以另外排了闭眼段，不依赖屏幕的那几段明说「可以闭上眼睛」，讲完再把他放回原位。

**`bbct` 还有一条：板书盖不住作业的时候，拿考纲当板书。**
板书跟作业常常对不齐，盖不住的那几道题背后照样有知识点，做题的时候一样会卡住。
办法是给它补一块板书：从作业本身判断这是哪一科哪一个单元，去查考纲，
把考纲条目当板书用，条目对应的真题就是这一段讲解的核心参考。
只发了作业、指令却写着 `bbct` 的，整份都走这一步：这时候考纲就是板书。
分界线一句话：**板书盖到的地方板书说了算，板书没盖到的地方考纲说了算，两处合起来把作业盖满。**

---

## 交付前还有一道工序：让子代理扮演学生，把每一处衔接问穿

写完不等于交得出去。规格在 [SKILL.md §1.85](plugins/dialogue-tutor/skills/dialogue-tutor/SKILL.md)，
顺序定死三道关，一道都不能提前：

1. **先把完整的第一版写完**，两半都写足，七幕六样齐全。
2. **用文风技能自检**，`writing-style` 与 `respectful-wording` 各跑一遍。
3. **自己判断已经可以交付了，再开子代理。** 边写边问会把这道工序变成打草稿。

子代理的人设写死成**学术水平中等且智商不高的高中生**。这个设定是有意的：
聪明的评审者会自己把缺口补上，于是缺口就留在了产物里。人设的五条特征照教育研究写，
出处列在规格那张表里（程序会做概念不会、不自发使用检查策略、小学阶段的基本动作就会卡住、
数学焦虑上升课堂参与下降、统计有假设检验概念与 p 值与 α 这三个自己的难处），
每一条都预测了子代理会在哪里断掉。

追问要有**学术对话感**：学生列出自己已经跟着走到第几步，指出卡在哪两步之间；
老师从本质入手，**以 8 为始、以 9 为终，中间推演出 8.1、8.2，最后落到 9**。
补出来的小步跟主推导是同一套规格，每个等号单独占一行，每一行旁边写出它凭什么成立，
在纸面上看不出哪一行是后来补的。每一个新出现的公式都是重灾区，
每一处不好理解的细节至少几轮。追问与解答进产物，跟行行讲解并列。

## 四份外部技能，各自在哪一步进来

[SKILL.md §1.86](plugins/dialogue-tutor/skills/dialogue-tutor/SKILL.md) 把调用的动作写死了，
四份技能各自的进入点、要读的文件、交出去什么、收回来什么都能在自查里点到：

| 技能 | 在哪一步进来 | 收回来什么 |
|---|---|---|
| `writing-style` | 第一版写完之后，开子代理之前 | 一张改动清单。它 §19 明写本技能是它的下位应用，冲突时以它为准；§16 给「讲稿」单列的那一行直接管着 `plustts` |
| `obra-superpowers` | 自查过关之后的自辩论，以及全文完成之后的整体评审 | 每一轮的追问、逐条裁定、一份台账。它的实现者 / 简报 / 评审者 / 发现 / 修复轮 / 定向复审逐个翻译到讲解稿上 |
| `emilkowalski-skills` | 产物是 HTML 的时候，做版面那一步 | 一张 Before / After / Why 三列表。`prefers-reduced-motion` 在这一档产物上是硬约束 |
| `manim` | 一张图的信息在于变化的过程，而不在最后的形状 | 一段动画加一段旁白稿，旁白稿并回讲解稿的对应位置 |

`manim` 那一条附带一句判据，把「能画图的地方别偷懒」那七类图劈成两半：
条件概率的样本空间缩小、连续性修正的 ±0.5、cdf 的升势与密度面积的对应，
这三样的意思全在变化的那一下，静态图截住的是变化之前和之后，中间那一下丢了。
其余四类形状就是全部内容，走内联 SVG 就够。
...
> **个性化教科书，文书想用什么，这个教科书就指哪打哪，语言通俗。且提示词完全没改。**
> <sub>——作者对这份实例的概括</sub>

最后那半句是这一节的分量所在：**技能原样跑，没有为这个领域添一句提示词。**
换领域的时候要动的是内容，不是规格。

「个性化教科书」这半句同样有实义：上半那六章不是一份通用大纲，
是照着那份文书要用到的东西裁出来的。文书里调用了能力进路，第一章就造福利的
度量空间；调用了自发秩序，第二章就造价格通道。**章目由下半决定**，
这跟 SKILL.md §1.75「下半会用到的那些公式，追溯得最深」是同一条规矩，
只是在这里决定的是整章的去留。
...
第④幕在那一份里走的是经典案例那一版：饥荒里的米在哪里、一块锡与一支铅笔、
社保号码那个实验。都跟那份文书毫无关系，正好拿来把对象本身讲透。
**例题那一条主要服务数学与统计**；换到手边没有现成题目的领域，
第④幕退回经典案例，或者拿一段跟本篇无关的样例照下半的规格拆一遍。
